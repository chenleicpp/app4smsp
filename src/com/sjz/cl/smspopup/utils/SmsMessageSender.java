...
import com.sjz.cl.smspopup.receiver.SmsReceiver;
...
public class SmsMessageSender {
	...
	private static final String TAG = "popup.SmsMessageSender";
	...
	public static final String[] MESSAGING_APP_ACTIVITIES = { MESSAGING_CONVO_CLASS_NAME, MESSAGING_COMPOSE_CLASS_NAME,
	...
	public SmsMessageSender(Context context, String[] dests, String msgText, long threadId) {
		...
		splitMessage = DEFAULT_SPLIT_MESSAGE;
		...
		for (int i = 0; i < mNumberOfDests; i++) {
			...
			if (splitMessage) {
				for (int j = 0; j < messageCount; j++) {
					Uri uri = null;
					try {
						uri = addMessage(mContext.getContentResolver(), mDests[i], messages.get(j), null, mTimestamp,
					...
					if (requestDeliveryReport) {
						deliveryReportIntent = PendingIntent.getBroadcast(mContext, 0, new Intent(
								MESSAGING_STATUS_RECEIVED_ACTION, uri).setClassName(MESSAGING_PACKAGE_NAME,
								MESSAGING_STATUS_CLASS_NAME), 0);
					}

					PendingIntent sentIntent = PendingIntent.getBroadcast(mContext, 0, new Intent(
							SmsReceiverService.ACTION_SMS_SENT, uri).setClass(mContext, SmsReceiver.class), 0);

					smsManager.sendTextMessage(mDests[i], mServiceCenter, messages.get(j), sentIntent,
				...
				for (int j = 0; j < messageCount; j++) {
					...
					sentIntents.add(PendingIntent.getBroadcast(mContext, 0, new Intent(
							SmsReceiverService.ACTION_SMS_SENT, uri).setClass(mContext, SmsReceiver.class), 0));
				}
				Log.d(TAG, "Sending message in " + messageCount + " parts");
	...
	private String getOutgoingServiceCenter(long threadId) {
		...
		try {
			cursor = mContext.getContentResolver().query(PopupUtil.SMS_CONTENT_URI, SERVICE_CENTER_PROJECTION,
